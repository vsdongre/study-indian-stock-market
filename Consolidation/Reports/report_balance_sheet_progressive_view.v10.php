<?php
// Consolidation/Reports/report_balance_sheet_progressive_view.v10.php
//
// Progressive Balance Sheet (Branch Wise) — user-selected Date1 vs Date2
//
// Params:
//   BranchID  (int)
//   dtDate1   (YYYY-MM-DD)
...
/**
 * Resolve the closest available EntryDate for a given BranchID and target date:
 *  1) Exact match
 *  2) Latest date in same month/year <= target
 *  3) Latest date overall <= target
 */
function resolveEntryDate(PDO $pdo, int $BranchID, string $dt): string {
    // 1) Exact match
    $stmt = $pdo->prepare("
    ...
    ");
    $stmt->execute([$BranchID, $dt, $dt, $dt]);
    $d = (string)$stmt->fetchColumn();
    if ($d !== '') return $d;
...
if ($BranchID <= 0
    ...
    echo "Invalid parameters. Use BranchID, dtDate1, dtDate2 (YYYY-MM-DD).";
    exit;
}
...
$entry1 = resolveEntryDate($pdo, $BranchID, $dtDate1);
$entry2 = resolveEntryDate($pdo, $BranchID, $dtDate2);

if ($entry1 === '' || $entry2 === '') {
    http_response_code(404);
    echo "No BalanceSheetBranch data found for BranchID={$BranchID} for the selected dates.";
    exit;
}
...
$stmt->execute([$entry1, $entry2, $BranchID, $entry1, $entry2]);
...
if ($export === 'csv') {
    ...
    header('Content-Disposition: attachment; filename="BalanceSheet_Branch_' . $BranchID . '_' . $entry1 . '_vs_' . $entry2 . '.csv"');
    $out = fopen('php://output', 'w');

    fputcsv($out, ['Progressive Balance Sheet — Branch Wise']);
...
<head>
  ...
  <title>Progressive Balance Sheet — Branch Wise</title>
  <style>
...
<div class="page">
  ...
  <div class="hdr">
    <p class="t1">Progressive Balance Sheet — Branch Wise &nbsp;|&nbsp; As On: <?php echo h($asOnText); ?></p>
    <div class="sub">
      <div>Branch: <b><?php echo h($BranchName); ?></b></div>
