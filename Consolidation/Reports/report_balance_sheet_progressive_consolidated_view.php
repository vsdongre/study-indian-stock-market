<?php
// Consolidation/Reports/report_balance_sheet_progressive_consolidated_view.php
//
// Progressive Balance Sheet (Consolidated — all branches) — user-selected Date1 vs Date2
//
// Behavior:
//   Aggregates BalanceSheetBranch across ALL branches.
//   For each date, resolve EntryDate as:
//     1) exact match (any branch has data for that date)
//     2) latest date in same month/year <= target
//     3) latest date overall <= target
//   Then sum BalBal across all branches for each resolved date.
//
// Params:
//   dtDate1   (YYYY-MM-DD)
//   dtDate2   (YYYY-MM-DD)
...
/**
 * Resolve the closest available entry date (consolidated — across all branches):
 *  1) Exact match
 *  2) Latest date in same month/year <= target
 *  3) Latest date overall <= target
 */
function resolveEntryDate(PDO $pdo, string $dt): string {
    // 1) Exact match
    $stmt = $pdo->prepare("
        SELECT TOP 1 CONVERT(varchar(10), CAST(dtDate AS date), 120) AS EntryDate
        FROM BalanceSheetBranch
        WHERE CAST(dtDate AS date) = CONVERT(date, ?, 120)
    ...
    $stmt = $pdo->prepare("
        SELECT TOP 1 CONVERT(varchar(10), CAST(dtDate AS date), 120) AS EntryDate
        FROM BalanceSheetBranch
        WHERE YEAR(CAST(dtDate AS date))  = YEAR(CONVERT(date, ?, 120))
...
if ($dtDate1 === '' || !preg_match('/^\d{4}-\d{2}-\d{2}$/', $dtDate1)
    || $dtDate2 === '' || !preg_match('/^\d{4}-\d{2}-\d{2}$/', $dtDate2)
) {
    http_response_code(400);
    echo "Invalid parameters. Use dtDate1, dtDate2 (YYYY-MM-DD).";
    exit;
}

$pdo = getPDOConnection();

$entry1 = resolveEntryDate($pdo, $dtDate1);
$entry2 = resolveEntryDate($pdo, $dtDate2);

if ($entry1 === '' || $entry2 === '') {
    http_response_code(404);
    echo "No BalanceSheetBranch data found for the selected dates.";
    exit;
}

// ── Fetch consolidated balances for both resolved entry dates ─────────────────
// Aggregates BalBal across ALL branches for each head
$sql = "
    SELECT
        h.HeadID,
        h.HeadGroupID,
    ...
    WHERE CAST(b.dtDate AS date) = ? OR CAST(b.dtDate AS date) = ?
    GROUP BY
...
$stmt->execute([$entry1, $entry2, $entry1, $entry2]);
...
if ($export === 'csv') {
    ...
    header('Content-Disposition: attachment; filename="BalanceSheet_Consolidated_' . $entry1 . '_vs_' . $entry2 . '.csv"');
    $out = fopen('php://output', 'w');

    fputcsv($out, ['Progressive Balance Sheet — Consolidated (All Branches)']);
...
<div class="page">
  ...
  <div class="hdr">
    <p class="t1">Progressive Balance Sheet — Consolidated (All Branches) &nbsp;|&nbsp; As On: <?php echo h($asOnText); ?></p>
    <div class="sub">
