<?php
// Consolidation/Reports/report_balance_sheet_progressive_home.v6.php
// Progressive Balance Sheet (Home) — Branch Wise, user selects two dates

require_once __DIR__ . '/../db.php';

$pdo = getPDOConnection();
$branches = $pdo->query("SELECT BranchID, BranchName FROM Branch ORDER BY BranchName")->fetchAll(PDO::FETCH_ASSOC);
...
$reportPath = "/Consolidation/Reports/report_balance_sheet_progressive_view.v10.php";
?>
<!doctype html>
<html lang="en">
<head>
...
<div class="wrap">
  <div class="cardX">
    <div class="headX">
      <div>
        <div class="title"><i class="fa fa-scale-balanced me-2 text-primary"></i>Progressive Balance Sheet — Branch Wise</div>
        <div class="hint">Select Branch + Date 1 + Date 2 (report will show both dates side-by-side)</div>
      </div>
        ...
        <div class="col-md-7">
          ...
          <select id="BranchID" class="form-select">
            ...
            <?php foreach ($branches as $b): ?>
              <option value="<?php echo (int)$b['BranchID']; ?>"><?php echo htmlspecialchars($b['BranchName'], ENT_QUOTES, 'UTF-8'); ?></option>
            <?php endforeach; ?>
          </select>
        </div>
...
<script>
  ...
  function buildUrl(mode) {
    let url = reportBase
      + '?BranchID=' + encodeURIComponent(elBranch.value)
  ...
  function validate() {
    hideAlert();
    if (!elBranch.value)    return showAlert('danger', 'Select Branch.'), false;
